#!/usr/bin/env python3
"""
API محسن للبحث في القوانين الفلسطينية
Enhanced Palestinian Legal Search API
"""
...
def health_check():
    ...
    return jsonify({
        ...
        'embeddings_ready': rag_system.embeddings is not None if rag_system else False
    })

@app.route('/api/search', methods=['POST'])
def search_laws():
    """البحث في القوانين"""
    if not rag_system or not rag_system.embeddings:
        return jsonify({
            'error': 'نظام RAG غير محمل أو Embeddings غير جاهزة',
            'status': 'error'
        }), 500
...
def initialize_rag_system(corpus_path: str, load_embeddings: bool = True):
    """تهيئة نظام RAG"""
    global rag_system
    ...
    try:
        ...
        if RAG_AVAILABLE and rag_system.model and load_embeddings:
...
def main():
    ...
    parser.add_argument('--no-embeddings', action='store_true',
                       help='عدم بناء Embeddings (للاختبار السريع)')
    
    args = parser.parse_args()
    
    # تهيئة النظام
    initialize_rag_system(args.corpus, load_embeddings=not args.no_embeddings)
